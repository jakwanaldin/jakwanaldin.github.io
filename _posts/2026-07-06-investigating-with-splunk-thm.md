...
> **Note for publishing:** this post references 13 screenshots at paths like `/assets/img/investigating-with-splunk-thm/01-....png`. Upload the accompanying `investigating-with-splunk-thm` image folder into your repo's `assets/img/` directory (create it if it doesn't exist) so the paths resolve correctly.
{: .prompt-tip }
...
![Initial search showing 12,256 events ingested into the main index](/assets/img/investigating-with-splunk-thm/01-initial-search-12256-events.png)
...
![EventID 4720 search returning one account creation event](/assets/img/investigating-with-splunk-thm/02-eventid-4720-search.png)
...
![Full account creation event details showing account attributes](/assets/img/investigating-with-splunk-thm/03-new-account-details.png)
...
![Sysmon EventID 12 registry event showing the SAM registry key for A1berto](/assets/img/investigating-with-splunk-thm/04-sysmon-registry-key.png)
...
![Full EventID 4688 process creation event with WMIC command line](/assets/img/investigating-with-splunk-thm/05-wmic-process-creation-event.png)
_EventID 4688 showing the remote WMIC process creation initiated from `James.browne`_

![Zoomed view of the WMIC command line creating the backdoor account](/assets/img/investigating-with-splunk-thm/06-wmic-commandline-zoom.png)
...
![Search for login events using the A1berto account returning zero results](/assets/img/investigating-with-splunk-thm/07-backdoor-login-search-zero-events.png)
...
![Search results for suspicious PowerShell command activity](/assets/img/investigating-with-splunk-thm/08-powershell-commands-search.png)
...
![Search for EventID 4103 returning 79 events](/assets/img/investigating-with-splunk-thm/09-eventid-4103-79-events.png)
...
![CyberChef input of the full Base64-encoded PowerShell payload](/assets/img/investigating-with-splunk-thm/10-cyberchef-base64-input.png)
_The full Base64-encoded PowerShell payload extracted from the 4103 event logs_

![CyberChef From Base64 and Remove null bytes recipe revealing the deobfuscated PowerShell script](/assets/img/investigating-with-splunk-thm/11-cyberchef-decoded-powershell.png)
_`From Base64` → `Remove null bytes` reveals the full WebClient/IEX download-and-execute chain_

![CyberChef decoding the smaller embedded Base64 string to reveal the C2 URL](/assets/img/investigating-with-splunk-thm/12-cyberchef-decoded-c2-url.png)
...
![CyberChef Defang URL recipe applied to the C2 URL](/assets/img/investigating-with-splunk-thm/13-cyberchef-defang-url.png)
